...
class JokerBomb(HandPattern):
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) == 2:
            ...
            if ranks == ["BJ", "RJ"]:
                return HandMatch(self.name, key=RANK_VALUE["RJ"], meta={}, priority=self.priority, size=2)
        return None
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) == 4:
            ...
            if len(cnt) == 1:
                ...
                return HandMatch(self.name, key=RANK_VALUE[r], meta={}, priority=self.priority, size=4)
        return None
...
class Single(HandPattern):
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) == 1:
            r = cards[0].rank()
            return HandMatch(self.name, key=RANK_VALUE[r], meta={}, priority=self.priority, size=1)
        return None
...
class Pair(HandPattern):
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) == 2:
            ...
            if len(cnt) == 1:
                r = next(iter(cnt.keys()))
                return HandMatch(self.name, key=RANK_VALUE[r], meta={}, priority=self.priority, size=2)
        return None
...
class Triple(HandPattern):
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) == 3:
            ...
            if len(cnt) == 1:
                r = next(iter(cnt.keys()))
                return HandMatch(self.name, key=RANK_VALUE[r], meta={}, priority=self.priority, size=3)
        return None
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) == 5:
            ...
            if sorted(cnt.values()) == [2, 3]:
                r = [k for k, v in cnt.items() if v == 3][0]
                return HandMatch(self.name, key=RANK_VALUE[r], meta={}, priority=self.priority, size=5)
        return None
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) >= 5:
            ...
            if len(set(ranks)) == len(ranks) and is_consecutive(ranks):
                return HandMatch(self.name, key=RANK_VALUE[ranks[-1]], meta={"length": len(ranks)}, priority=self.priority, size=len(ranks))
        return None
    def same_shape(self, a: HandMatch, b: HandMatch) -> bool:
        ...
        if len(cards) >= 6 and len(cards) % 2 == 0:
            ...
            if all(v == 2 for v in cnt.values()):
                ...
                if is_consecutive(seq):
                    return HandMatch(self.name, key=RANK_VALUE[seq[-1]], meta={"length_pairs": len(cards) // 2}, priority=self.priority, size=len(cards))
        return None
    def same_shape(self, a: HandMatch, b: HandMatch) -> bool:
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) >= 6 and len(cards) % 3 == 0:
            ...
            if all(v == 3 for v in cnt.values()):
                seq = sorted(cnt.keys(), key=lambda r: RANK_VALUE[r])
                if is_consecutive(seq):
                    return HandMatch(self.name, key=RANK_VALUE[seq[-1]], meta={"length_triples": len(cards) // 3}, priority=self.priority, size=len(cards))
        return None
    def same_shape(self, a: HandMatch, b: HandMatch) -> bool:
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) == 6:
            ...
            if sorted(cnt.values()) == [1, 1, 4]:
                ...
                return HandMatch(self.name, key=RANK_VALUE[r], meta={}, priority=self.priority, size=6)
        return None
    ...
    def match(self, cards: List[Card]) -> Optional[HandMatch]:
        if len(cards) == 8:
            ...
            if sorted(cnt.values()) == [2, 2, 4]:
                r = [k for k, v in cnt.items() if v == 4][0]
                return HandMatch(self.name, key=RANK_VALUE[r], meta={}, priority=self.priority, size=8)
        return None
